# 📋 TIẾN ĐỘ TRIỂN KHAI — Chatbot Nông Nghiệp AI (Phiên bản )

> **Cập nhật lần cuối:** 2026-08-23 (Phiên Nâng Cấp  — Tối Ưu Pipeline & Gemini Pool)  
> **Mục đích:** Theo dõi toàn bộ tiến độ dự án. Đọc file này để tiếp tục làm việc, vận hành hoặc khôi phục hệ thống khi cần.  
> **Trạng thái hiện tại:** 🟢 **HOÀN THÀNH CẬP NHẬT VÀ NÂNG CẤP HỆ THỐNG PHIÊN BẢN **
...
| **LLM Provider & Models** | **Google Gemini** với cơ chế **Key Rotation Pool** (`GEMINI_API_KEY_1..4`). Router: `gemini-3.1-flash-lite`, Synthesis: `gemini-3.5-flash` |
| **Gemini Key Pool Manager** | Tự động xoay vòng Round-Robin qua tối đa 4 API Keys; tự động cách ly key bị 429/Rate-limit (cooldown 10s tự khôi phục); endpoint giám sát real-time `/api/admin/key-status` |
...
| **Dọn dẹp Tự động** | Tự động xóa file và pending confirmation quá hạn 24 giờ khi server khởi động |
| **Authentication & Phân quyền** | Đăng ký, Đăng nhập (kiểm tra tài khoản khóa `is_blocked`), Khởi tạo tài khoản `admin` / `admin` mặc định |
| **Admin Dashboard** | Trang Quản lý Hệ thống độc lập (`/admin` hoặc `admin.html`) với 3 tab: Upload Dữ liệu thô (Data Pipeline), Quản lý Người dùng (Chặn/Bỏ chặn/Xoá tài khoản) & Giám sát Gemini Key Status Pool |
| **User Chat History** | Lịch sử trò chuyện riêng biệt cho từng tài khoản người dùng trong PostgreSQL |
| **Chat Interface UI** | Giao diện hiện đại, sạch sẽ; loại bỏ gợi ý mặc định & nút Like/Dislike; hiển thị rõ nguồn tri thức theo tầng (Layer 1 Fact, Layer 2 Knowledge Graph, Layer 3 RAG ChromaDB) |
...
├── data.pdf                            <- File dữ liệu tài liệu mẫu
├── backend\
│   ├── app.py                          <- FastAPI Server  (Auth, Admin APIs, Chat API, Key Status)
│   ├── config.py                       <- Cấu hình hệ thống (Key Pool 1..4, DB URL, Paths)
│   ├── db\
...
│   └── utils\
│       ├── gemini_client.py            <- GeminiKeyManager (Pool 4 keys, auto-retry, rate-limit cooldown)
│       └── text_utils.py               <- Tiện ích xử lý văn bản
├── frontend\
│   ├── index.html                      <- Màn hình Đăng nhập/Đăng ký & Giao diện Chat chính
│   ├── admin.html                      <- Trang Quản lý Hệ thống (Admin Dashboard chuyên biệt)
│   ├── style.css                       <- CSS thiết kế giao diện hiện đại, responsive
│   └── app.js                          <- Logic Auth, Chat Stream, Admin Upload (handling 409 modals), User Mgr
├── scripts\
...
### 🟢 2. Quản Lý Pool Gemini API Key (Key Rotation Pool)
...
- [x] Phân loại lỗi chính xác: lỗi 429/Rate Limit (tự động cooldown 10 giây), lỗi key không hợp lệ 401/403 (cách ly vĩnh viễn).
- [x] Tạo endpoint API `/api/admin/key-status` để Admin theo dõi trạng thái, tổng số lượt gọi và số lỗi của từng key theo thời gian thực.

### 🟢 3. Hệ Thống Xác Thực & Quản Lý Tài Khoản (Authentication & Admin)
...
- [x] Trang Admin Dashboard độc lập (`admin.html`) với tính năng:
  ...
  - Xem bảng theo dõi sức khỏe Gemini API Key Pool.

### 🟢 4. Giao Diện Người Dùng & Trải Nghiệm Chatbot (Chat Interface)
...
- [x] Loại bỏ các câu hỏi gợi ý mặc định và nút Like/Dislike rườm rà.
- [x] Hiển thị chi tiết tầng tri thức phản hồi (Tầng 1 — Fact Store, Tầng 2 — Knowledge Graph, Tầng 3 — Document Store RAG ChromaDB).

### 🟢 5. Kiến Trúc Tri Thức 3 Tầng (3-Layer Hybrid Architecture)
